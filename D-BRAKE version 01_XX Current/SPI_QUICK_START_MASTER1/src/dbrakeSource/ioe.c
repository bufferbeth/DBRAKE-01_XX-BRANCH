...
static void IoePinWrite( uint8_t port, uint8_t pin, uint8_t enable )
{
    ...
    SX1272Read( port, &data );
    if( enable == 1 )
    {
        SX1272Write( port, data | pin );
    }
    else
    {
        SX1272Write( port, data & ( ~pin ) );
...
void IoeInit( void )
{
    ...
    SX1272Write( RegClock, 0x40 );

    // SHDN1 SHDN0 SIOEI READY RXTX  DIO5  DIO4  DIO3
    //  OUT   OUT   IN    IN    IN    IN    IN    IN
    SX1272Write( RegDirA, 0x3F );
    
    // N/A   N/A   N/A   SPARE CTX   CPS   CSD   ANT_S
    //  IN    IN    IN    IN    OUT   OUT   OUT   OUT
    SX1272Write( RegDirB, 0xF0 );
    
#if defined( USE_FEM_BOARD )
    SX1272Write( RegOpenDrainB, 0x0F );
    //SX1272Write( RegInputDisableB, 0x0F );
#endif

    SX1272Write( RegSenseHighA, 0x0F );
    SX1272Write( RegSenseLowA, 0xFF );

    SX1272Write( RegSenseHighB, 0x00 );
    SX1272Write( RegSenseLowB, 0x00 );
    
    SX1272Write( RegInterruptMaskA, 0xC4 );
    SX1272Write( RegInterruptMaskB, 0xFF );

    SX1272Write( RegDataA, 0x00 );
    SX1272Write( RegDataB, 0xEF );
    ...
    SX1272Write( RegDirA, 0xF8 );
    SX1272Write( RegOpenDrainA, 0x07 );
    //SX1272Write( RegInputDisableA, 0x07 );
    
    // GPIO7 GPIO6 GPIO5 GPIO4 GPIO3 GPIO2 GPIO1 GPIO0
    //  IN    IN    IN    IN    IN    IN    IN    IN  
    SX1272Write( RegDirB, 0xFF );

    SX1272Write( RegSenseHighA, 0x00 );
    SX1272Write( RegSenseLowA, 0xC0 );

    SX1272Write( RegSenseHighB, 0x00 );
    SX1272Write( RegSenseLowB, 0x00 );
    
    SX1272Write( RegInterruptMaskA, 0xF7 );
    SX1272Write( RegInterruptMaskB, 0xFF );

    SX1272Write( RegDataA, 0x07 );
    SX1272Write( RegDataB, 0x00 );
