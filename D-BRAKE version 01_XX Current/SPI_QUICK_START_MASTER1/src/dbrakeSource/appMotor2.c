...
//----- boc 01_38_#3	
	...
	//----------------------------
	if ((action == EXTENDING)||(action == RETRACTING))
	{
...
//											brakeStatus.BrakeState &= ~BRAKESTATE_NOTSETUP;
											...
											MotorOff();
											brakeSupTime =BRAKESUPTIME;  //BRAKESUPTIME; //V01_27
											MotorCCW(); //V01_27
											//------------v1.05 boc
...
// error handling states
		...
		case BRAKESTATE_ERROR:
		{
			//---------------------
			// make sure at home 
			if ((hlimitState!=0)&&(homeLimit == HOME_OUT)&&(brakeSupTime ==0)) //V01_26
			{
 					MotorCCW();
					brakeSupTime = BRAKESUPTIME;
					brakeState = BRAKESTATE_ERROR_FINAL; 
		...
		{
			if (((brakeChange & BRAKECHANGE_SUPTIME)!=0)||
				((hlimitState ==0)||(homeLimit == HOME_IN))) //V01_27			
			{
				MotorOff();
				brakeChange &= ~BRAKECHANGE_SUPTIME; 
			}
